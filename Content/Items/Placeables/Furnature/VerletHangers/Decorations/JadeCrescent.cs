...
public class JadeCrescent : Decoration, ILocalizedModType
{
    ...
    public override int DecorID => DecorationID.JadeCrescent;

    public override void UpdateDecoration(Vector2[] segmentPositions)
    {
        Lighting.AddLight(segmentPositions[^1], Color.Lime.ToVector3() * 0.5f);
    }
    ...
    public override void DrawAlongVerlet(SpriteBatch spriteBatch, int index, Vector2[] segmentPositions)
    {
        ...
        if (index % 3 == 0 && index != 0 && index != segmentPositions.Length - 1)
        {
            ...
            spriteBatch.Draw(tex, drawPos - Main.screenPosition, frame, Color.White, rotation - PiOver2, origin, 1f, (index / 3) % 2 == 0 ? SpriteEffects.FlipHorizontally : 0, 0);
        }
    }

    public override void DrawOnVerletEnd(SpriteBatch spriteBatch, Vector2[] segmentPositions)
    {
        Texture2D texture = ModContent.Request<Texture2D>("Windfall/Content/Items/Placeables/Furnature/VerletHangers/Decorations/JadeCrescent").Value;
