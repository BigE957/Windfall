        ...
        switch (AIState)
        {
            ...
            case States.IdolEnactment:
                ...
                if (aiCounter < 1200)
                {
                    ...
                    if (aiCounter % (CalamityWorld.death ? 30 : CalamityWorld.revenge ? 34 : Main.expertMode ? 36 : 40) == 0)
                    {
                        ...
                        for (int i = 0; i <= 20; i++)
                        {
                            SelenicMetaballParticle.SpawnParticle(NPC.Center, Main.rand.NextVector2Circular(10f, 10f), 40 * Main.rand.NextFloat(1.5f, 2.3f));
                        }
            ...
            case States.DarkBarrage:
                ...
                if (aiCounter > 1100 || !NPC.AnyNPCs(ModContent.NPCType<OratorHand>()))
                {
                    ...
                    SelenicMetaballParticle.SpawnParticle(NPC.Center + homeIn * 150f, Main.rand.NextVector2Circular(5f, 5f), 1.5f * ((aiCounter - 1100) / 2));
            ...
            case States.DarkStorm:
                ...
                if (aiCounter > 120 && NPC.Center.Y < target.Center.Y - 50)
                {
                    ...
                    if (aiCounter % AttackFrequency == 0 && aiCounter < 690)
                    {
                        ...
                        for (int i = 0; i < 10; i++)
                        {
                            SelenicMetaballParticle.SpawnParticle(new(NPC.Center.X, NPC.Center.Y - 50), Main.rand.NextVector2Unit(0, -Pi) * Main.rand.NextFloat(0f, 15f), 25 * Main.rand.NextFloat(1f, 2f));
                        }
                        if (Main.netMode != NetmodeID.MultiplayerClient)
                        {
                            Vector2 spawnPos = NPC.Center + NPC.velocity;
                ...
                if (aiCounter >= 0)
                {
                    ...
                    if (aiCounter < (attackLength * 0.66f))
                    {
                        ...
                        if (Main.expertMode && aiCounter % (CalamityWorld.revenge ? 90 : 120) == 0 && aiCounter > 30)
                        {
                            ...
                            for (int i = 0; i < 20; i++)
                            {
                                SelenicMetaballParticle.SpawnParticle(NPC.Center, (border.Center - NPC.Center).SafeNormalize(Vector2.Zero).RotatedByRandom(Pi / 6) * Main.rand.NextFloat(0f, 25f), 30 * Main.rand.NextFloat(1f, 2f));
                            }
                            for (int i = 1; i < 8; i++)
                            {
                                if (Main.netMode != NetmodeID.MultiplayerClient)
            ...
            case States.DarkFlight:
                ...
                if(aiCounter > 350)
                {
                    SelenicMetaballParticle.SpawnParticle(NPC.Center + (target.Center - NPC.Center).SafeNormalize(Vector2.Zero) * 80f, Main.rand.NextVector2Circular(5f, 5f), Lerp(0f, 1f, (aiCounter - 400) % 120));
                    ...
                    else
                    {
                        ...
                        if (attackLength == -1)
                        {
                            ...
                            if (NPC.Center.Y > wallTop && attackLength == -1)
                            {
                                ...
                                dashing = false;
                                for (int i = 0; i < 32; i++)
                                    SelenicMetaballParticle.SpawnParticle(NPC.Center + new Vector2(Main.rand.NextFloat(-64, 64), 160), Vector2.UnitY * Main.rand.NextFloat(4f, 18f) * -1, Main.rand.NextFloat(110f, 130f));
                        ...
                        else if (aiCounter < attackLength + 500)
                        {
                            if (aiCounter < attackLength + 450)
                            {
                                ...
                                if (Math.Abs(NPC.velocity.X) > 8)
                                    if (NPC.velocity.X < 0)
                                        ...
                                        NPC.velocity.X = 8;
                                SelenicMetaballParticle.SpawnParticle(NPC.Center + Vector2.UnitX * Main.rand.NextFloat(-48, 48), Vector2.UnitY * Main.rand.NextFloat(8f, 12f) * -1, Main.rand.NextFloat(90f, 110f));
                            ...
                            {
                                for (int i = 0; i < 2; i++)
                                    SelenicMetaballParticle.SpawnParticle(NPC.Center + new Vector2(Main.rand.NextFloat(-64, 64), 64), Vector2.UnitY * Main.rand.NextFloat(10f, 14f) * -1, Main.rand.NextFloat(110f, 130f));
                                if (aiCounter == attackLength + 450)
                                {
                                    dashing = true;
                                    SoundEngine.PlaySound(DashWarn);
                        ...
                        else
                        {
                            ...
                            if (NPC.position.Y < wallTop && NPC.oldPosition.Y >= wallTop)
                            {
                                ...
                                for (int i = 0; i < 32; i++)
                                    SelenicMetaballParticle.SpawnParticle(NPC.Center + new Vector2(Main.rand.NextFloat(-64, 64), 64), Vector2.UnitY * Main.rand.NextFloat(4f, 24f) * -1, Main.rand.NextFloat(110f, 130f));
            ...
            case States.Spawning:
                ...
                if(aiCounter > 90 && aiCounter <= 150)
                {
                    ...
                    for (int i = 0; i < 12; i++)
                        SelenicMetaballParticle.SpawnParticle(NPC.Center + new Vector2(128 + Main.rand.NextFloat(-width, width), Main.rand.NextFloat(-24, 0)).RotatedBy(-PiOver4), new Vector2(Main.rand.Next(-2, 2), Main.rand.Next(1, 5) * SineInEasing(ratio)).RotatedBy(-PiOver4), Main.rand.NextFloat(15f, 25f) * (ratio * 2f));
                }
                if (aiCounter == 150)
                {
                    SoundEngine.PlaySound(SoundID.DD2_EtherianPortalDryadTouch, NPC.Center);
                    ...
                    for (int i = 0; i < 24; i++)
                        SelenicMetaballParticle.SpawnParticle(spawnPos + new Vector2(Main.rand.NextFloat(-64, 64), -32).RotatedBy(-PiOver4), Vector2.UnitY.RotatedBy(-PiOver4) * Main.rand.NextFloat(2f, 18f), Main.rand.NextFloat(110f, 130f));
                }
