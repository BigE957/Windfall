...
internal class ChefMenuUIState : UIState
{
    ...
    public override void OnInitialize()
    {
        ...
        {
            Item food = new(LunarCultBaseSystem.MenuFoodIDs[i]);
            Asset<Texture2D> foodTexture = ModContent.Request<Texture2D>(food.ModItem.Texture);
            UIImage foodIcon = new(foodTexture);
            SetRectangle(foodIcon, left: 40f - (foodIcon.Width.Pixels / 2f), top: 40f * (i + 2), width: foodIcon.Width.Pixels, height: foodIcon.Height.Pixels);
            UIPanel.Append(foodIcon);

            UIText foodName = new(food.Name);
