    ...
    public static void HandlePacket(Mod mod, BinaryReader reader, int whoAmI)
    {
        try
        {
            ...
            switch (msgType)
            {
                ...
                #endregion
                default:
                    Windfall.Instance.Logger.Error($"Failed to parse Windfall packet: No Calamity packet exists with ID {msgType}.");
        ...
        catch (Exception e)
        {
            if (e is EndOfStreamException eose)
                Windfall.Instance.Logger.Error("Failed to parse Windfall packet: Packet was too short, missing data, or otherwise corrupt.", eose);
            else if (e is ObjectDisposedException ode)
                Windfall.Instance.Logger.Error("Failed to parse Windfall packet: Packet reader disposed or destroyed.", ode);
            else if (e is IOException ioe)
                Windfall.Instance.Logger.Error("Failed to parse Windfall packet: An unknown I/O error occurred.", ioe);
            else
